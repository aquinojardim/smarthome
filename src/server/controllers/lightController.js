...
lightController.postLight = (req, res, next) => {
  ...
  try {
    fs.writeFileSync(path.resolve(__dirname, '../database/lights.json'), JSON.stringify(res.locals.data.lights), 'UTF-8');
  }
  catch {
    return next( {
      log: 'lightController.postLight: ERROR: Error writing to lights.json file',
...
lightController.deleteLight = (req, res, next) => {
  ...
  try {
    ...
    return next( {
      log: 'lightController.deleteLight: ERROR: Error writing to lights.json file',
...
lightController.allLight = (req, res, next) => {
  ...
  try {
    fs.writeFileSync(path.resolve(__dirname, '../database/lights.json'), JSON.stringify(obj), 'UTF-8');
  }
  catch {
    return next( {
