...
export default function Thermostat(props) {
  ...
  const min = restrictToRange(
    Math.round(
      ((actualMinValue - props.minValue) / rangeValue) * props.numTicks
    ),
    0,
    props.numTicks - 1
  );
  const max = restrictToRange(
    Math.round(
      ((actualMaxValue - props.minValue) / rangeValue) * props.numTicks
    ),
    0,
    props.numTicks - 1
  );
  ...
  let degs =
    (tickDegrees * (peggedValue - props.minValue)) / rangeValue - offsetDegrees;
